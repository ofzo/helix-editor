...
impl TerminalPane {
    ...
    fn key_to_bytes(&self, key: &KeyEvent) -> Option<Vec<u8>> {
        ...
        // When DECCKM (Application Cursor Keys) is active, arrow keys use
        // SS3 sequences (\x1bO) instead of CSI sequences (\x1b[).
        // Programs like less, git log, man, etc. enable this mode.
        ...
        let bytes = match key.code {
            ...
            KeyCode::Up if app_cursor => b"\x1bOA".to_vec(),
            KeyCode::Down if app_cursor => b"\x1bOB".to_vec(),
            KeyCode::Right if app_cursor => b"\x1bOC".to_vec(),
            KeyCode::Left if app_cursor => b"\x1bOD".to_vec(),
